...
namespace JobsPlatform.BusinessLogic.CRUD
{
    public static class EmployeerCrud
    {
        ...
        public static async Task CreateEmployeerTable()
        {
            ...
            query = "CREATE TABLE IF NOT EXISTS EMPLOYEER (id INTEGER PRIMARY KEY AUTOINCREMENT,name VARCHAR(30) NOT NULL,identifier INTEGER NOT NULL, userID INTEGER NOT NULL ,FOREIGN KEY (userID) REFERENCES USER(id) ON DELETE CASCADE)";
        ...
        public static async Task ReadEmployeerTable()
        {
            ...
            await using (SqliteDataReader reader = cmd.ExecuteReader())
            {
                while (reader.Read())
                {
                    ...
                    Database.everything.Add(new Employeer(name, id, identifier, uID));
                }
            }
            await Database.conn.CloseAsync();
        }
        public static async Task UpdateEmployeerTable(string? name, int? userID, int? employeerID)
        {
            await Database.conn.OpenAsync();
            query = $"UPDATE EMPLOYEER SET name='{name}', userID='{userID}' WHERE id='{employeerID}'";
            SqliteCommand cmd = new SqliteCommand(query, Database.conn);
            await cmd.ExecuteNonQueryAsync();
            await Database.conn.CloseAsync();
        }
        ...
        public static async Task InsertEmployeer(string? name, int? identifier, int? userID)
        {
            string query = $"INSERT INTO EMPLOYEER(name,identifier,userID) VALUES ('{name}','{identifier}','{userID}')";
            await Database.conn.OpenAsync();
            SqliteCommand cmd = new SqliteCommand(query, Database.conn);
            await cmd.ExecuteNonQueryAsync();
            await Database.conn.CloseAsync();
